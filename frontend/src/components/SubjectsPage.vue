      ...
      handleRouteQuery() {
        ...
        if (subjectCode && this.subjects.length > 0) {
          ...
          if (subject) {
            ...
            this.searchQuery = subject.name; // Populate search input with the subject name
            ...
            // Handle course search within the selected subject
            if (courseSearch && subject.courses) {
              const matchingCourse = subject.courses.find((course) =>
                course.title.toLowerCase().includes(courseSearch.toLowerCase())
              );

              if (matchingCourse) {
                this.searchQuery = matchingCourse.title; // Update search query with course title
